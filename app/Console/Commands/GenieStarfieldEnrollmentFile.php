...
class GenieStarfieldEnrollmentFile extends Command
{
     ...
     * @var string
     */
    protected $signature = 'Genie:StarfieldEnrollmentFile {--mode=} {--no-email} {--show-sql} {--start-date=} {--end-date=}';
    ...
    protected $distroList = [
        'live' => ['DXC_AutoEmails@dxc-inc.com', 'tkrapf@townsquareenergy.com','SalesSupport@genieretail.com'],
    ...
    public function handle()
    {
        ...
        1 // TPVs only
            ...
            "((service_state IN ('PA') AND product_utility_external_id IN ('PGW')) 
            OR (service_state IN ('IL') AND product_utility_external_id IN ('PEOPGAS', 'NSHORE')) 
            OR (service_state IN ('OH') AND product_utility_external_id IN ('VEDO', 'DEOHG', 'DPL') AND NOT ISNULL(contracts)) 
            OR (service_state IN ('MI') AND product_utility_external_id IN ('DTEGAS', 'CONSGAS') AND NOT ISNULL(contracts)))"
        )->orderBy(
            'interaction_created_at'
        );
    ...
    public function sendEmail(string $message, array $distro, array $files = array())
    {
        ...
        if ('production' != config('app.env')) {
            $subject = $subjectBrand . ' - Starfield File Generation (' . config('app.env') . ') '
